...
class Galaxy(object):
    ...
    def convertMagnitudes(self):
        ...
        noObs = np.where(self.mag_data==self.config.no_observe_value)[0]
        for no in noObs:
            self.flux_data[no] = 0.
        ...
        noDet = np.where(self.mag_data==self.config.no_detect_value)[0]
        for nd in noDet:
            self.flux_data[nd] = 0.
        ...
        seen = [(self.mag_data!=self.config.no_observe_value) * (self.mag_data!=self.config.no_detect_value)]
        notSeen = [~((self.mag_data!=self.config.no_observe_value) * (self.mag_data!=self.config.no_detect_value))]
